-- ============================================================
-- Portfolio Analysis — BigQuery Variant
-- Original: Portfolio_analysis.sql (PostgreSQL)
-- These are the 8 Power BI / Looker Studio dataset queries.
--
-- Key BQ adaptations:
--   • FILTER (WHERE ...) → COUNTIF(condition)
--   • ::numeric / ::INT casting → removed (BQ infers types natively)
--   • NULLIF() → identical syntax in BQ
--   • || string concat → CONCAT() in BQ
--   • EXTRACT() → identical in BQ
--   • Window functions (OVER()) → identical in BQ
-- ============================================================


-- ============================================================
-- QUERY 1: MAIN FACT TABLE
...
SELECT
    ...
    c.customer_id,
    CONCAT(c.first_name, ' ', c.last_name)             AS customer_name,  -- BQ uses CONCAT(), not ||
    c.age,
    c.gender,
    ...
    c.employment_type,
    c.monthly_income,
    ROUND(l.loan_amount / NULLIF(c.monthly_income, 0), 2) AS lti_ratio,
    CASE
...
SELECT
    ...
    ROUND(SUM(r.paid_amount) * 100.0 /
        NULLIF(SUM(r.emi_amount), 0), 2)                AS collection_efficiency,
    ROUND(AVG(r.days_past_due), 1)                      AS avg_days_past_due
FROM `your_project.your_dataset.loans` l
JOIN `your_project.your_dataset.repayments` r ON l.loan_id = r.loan_id;
...
SELECT
    ...
    ROUND(SUM(r.paid_amount) * 100.0 /
        NULLIF(SUM(r.emi_amount), 0), 2)                AS collection_efficiency,
    ROUND(AVG(r.days_past_due), 1)                      AS avg_dpd
FROM `your_project.your_dataset.loans` l
JOIN `your_project.your_dataset.branches`   b ON l.branch_id = b.branch_id
JOIN `your_project.your_dataset.repayments` r ON l.loan_id   = r.loan_id
...
SELECT
    ...
    ROUND(COUNT(DISTINCT CASE
        ...
        COUNT(DISTINCT l.loan_id), 2)                   AS write_off_rate,
    ROUND(SUM(r.paid_amount) * 100.0 /
        NULLIF(SUM(r.emi_amount), 0), 2)                AS recovery_rate,
    ROUND(AVG(r.days_past_due), 1)                      AS avg_dpd,
    -- PostgreSQL FILTER clause → BigQuery COUNTIF()
    COUNTIF(r.payment_status = 'Missed')                AS missed_payments,
    COUNTIF(r.payment_status = 'Paid')                  AS paid_payments,
    COUNTIF(r.payment_status = 'Partially Paid')        AS partial_payments
FROM `your_project.your_dataset.loans` l
JOIN `your_project.your_dataset.repayments` r ON l.loan_id = r.loan_id
...
SELECT
    ...
    ROUND(AVG(l.loan_amount) /
        NULLIF(AVG(c.monthly_income), 0), 2)            AS lti_ratio,
    COUNT(DISTINCT CASE
        WHEN l.loan_status = 'Written-Off'
        THEN l.loan_id END)                             AS written_off,
    ROUND(COUNT(DISTINCT CASE
...
SELECT
    ...
    ROUND(SUM(paid_amount) * 100.0 /
        NULLIF(SUM(emi_amount), 0), 2)                  AS recovery_rate
